...
import type { WorkoutSet, Exercise, MuscleGroup } from '@/types/entities';
...
import { getLastSetsForExercise } from '@/lib/api';
...
export function ExerciseBlock({
  ...
  const { data: lastSets } = useQuery({
    ...
    queryFn: () => getLastSetsForExercise(sessionExercise.exerciseId, sessionExercise.id),
    staleTime: Infinity,
  });
  ...
  const lastSetMap = useMemo(() => {
    ...
    for (const s of lastSets ?? []) {
      if (!map.has(s.order)) map.set(s.order, s);
    }
    return map;
  }, [lastSets]);
  ...
  const completedSets = useMemo(() => sets?.filter((s) => s.completedAt).length ?? 0, [sets]);
  const totalSets = sets?.length ?? 0;

  function handleAddSet() {
    const lastSet = sets?.[sets.length - 1];
    addSet.mutate({ sessionExerciseId: sessionExercise.id, fromSet: lastSet, order: (sets?.length ?? 0) + 1 });
  }
      ...
      <div className="flex items-center gap-2 pl-4 pr-3 pt-3 pb-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {totalVolume > 0 && (
              <span className="primary-pill">{formatVolume(totalVolume, exercise.type)}</span>
            )}
          </div>
          <div className="flex items-center gap-2 mt-0.5">
            {totalSets > 0 && (
              <span className="text-[11px] text-text-muted font-mono">{completedSets}/{totalSets} sets</span>
            )}
          </div>
        </div>
